...
function draw() {
 ...
 if(gameState==="PLAY"){
  ...
  text("PLAYER LIVES: " +playerlife,width/2.19,height/20);

 }
 if(mousePressedOver(about)){
   textSize(18);
   fill("yellow");
   text("*Use the Arrow Topkeys to move the player, collect the points*",80, 90);
   text("*Use the  Top Arrow keys to move the player UP*",80, 120);
   text("*Use the  Down Arrow keys to move the player Down*",80, 150);
   text("*Use the  Right Arrow keys to move the player Forward/Right*",80, 180);
   text("*Use the  Left Arrow keys to move the player Background/Left*",80, 210);
 }
